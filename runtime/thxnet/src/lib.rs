...
use thxnet_runtime_constants::{currency::*, fee::*, staking, time::*};
...
impl_runtime_weights!(thxnet_runtime_constants);
...
parameter_types! {
	...
	pub const BasicDeposit: Balance = deposit(5, 0);
	pub const FieldDeposit: Balance = deposit(0, 0);
	pub const SubAccountDeposit: Balance = deposit(0, 0);
...
impl ProcessMessage for MessageProcessor {
	...
	fn process_message(
		...
		xcm_builder::ProcessXcmMessage::<
			...
			RuntimeCall,
		>::process_message(message, xcm::latest::Junction::Parachain(para.into()), meter, id)
	}
}
...
parameter_types! {
	...
	pub LeasePeriod: BlockNumber = prod_or_fast!(20 * 365 * DAYS, 20 * 365 * DAYS, "DOT_LEASE_PERIOD");
